...
public class UDP_Handling : MonoBehaviour
{
    ...
    static UdpClient verticalAxisListener;
    static IPEndPoint verticalAxisGroupEP;

    static gameData gData = new gameData();
    ...
    void Start(){
        ...
        verticalAxisListener = new UdpClient(3500);
        verticalAxisGroupEP = new IPEndPoint(IPAddress.Any, 3500);
        verticalAxisListener.Client.ReceiveBufferSize = 0;
        verticalAxisListener.Client.ReceiveTimeout = 15;
    ...
    void Update()
    {
        ...
        UDPsend("G|" + data, 2300);
        UDPsend("G|" + Ztarget.ToString(), 4578);
    ...
    void ListenForVerticalAxis()
    {
        // Debug.Log("Getting Vertical Axis Data");
        try
        {
            byte[] bytes = verticalAxisListener.Receive(ref groupEP);
            string[] message = ($"{Encoding.ASCII.GetString(bytes, 0, bytes.Length)}").Split('|');
            z2pos = float.Parse(message[1]);
            // Debug.Log("Z Position: " + z2pos.ToString());
        }
        catch{ }
    }
